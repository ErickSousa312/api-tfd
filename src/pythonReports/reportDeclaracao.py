...
def gerar_relatorio_pdf(dados):
    caminho_completo = os.path.join(os.path.dirname(
        __file__), 'Declaracao de Comparecimento.pdf')
...
# DADOS GERAIS
    ...
    c.rect(205, 741, 195, 0.9, stroke=0, fill=1)
    ...
    c.roundRect(465, 730, 100, 35,
                rect_corner_radius, stroke=1, fill=0)
    c.setFont("Helvetica-Bold", 8)
    c.drawString(470, 756, "Nº do Atendimento")
    c.setFont("Helvetica-Bold", 9)
    c.drawString(490, 740, str(f"Nº {dados['_id']}"))
    ...
    texto = AjustarTexto(str(f"                 Declaro para devidos fins que {dados['IdPaciente']['NomePaciente']}, portador do RG Nº {dados['IdPaciente']['NumeroRG']} {dados['IdPaciente']['OrgaoEmissor']}/{dados['IdPaciente']['UF']} CPF Nº \
                            {dados['IdPaciente']['NumeroCPF']}, compareceu setor de Tratamento Fora de Domicílio - TFD, no período MATUTINO na presente \
data. \
                            "), 120)
    y = 689
    for linha in texto:
     ...
     # Dados Atendimento
    c.roundRect(30, 480, 540, 65, rect_corner_radius, stroke=1, fill=0)
    c.setFont("Helvetica-Bold", 12)
    c.drawString(CentralizarTexto("Dados do Atendimento", 12, c),
                 535, "Dados do Atendimento")

    # Nome funcionario
    c.roundRect(34, 484, 300, 47, rect_corner_radius, stroke=1, fill=0)
    c.setFont("Helvetica", 8)
    c.drawString(38, 523, "Nome do funcionário responsável pleo atendimento: ")
    c.setFont("Helvetica", 10)
    c.drawString(38, 487, "Atendente TFD de marabá ")

    # Data do atendimento
    c.roundRect(465, 510, 85, rect_height,
                rect_corner_radius, stroke=1, fill=0)
    c.setFont("Helvetica", 8)
    c.drawString(470, 525, "Data do Atendimento")
    c.setFont("Helvetica", 9)
    c.drawString(470, 514, str(data_formatada_numero))
